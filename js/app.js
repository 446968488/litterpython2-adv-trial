...
(function () {
  ...
  // ===== 语音风格：升阶版固定「幽默·梗王」一档；音色提供男声/女声可选（设置模块切换）=====
  ...
  // 音色：女声(晓晓 XiaoxiaoNeural) / 男声(云希 YunxiNeural)，设置模块切换，默认女声
  const VOICE_GENDER_KEY = 'course_voice_gender';
  function loadGender() { return localStorage.getItem(VOICE_GENDER_KEY) === 'm' ? 'm' : 'f'; }
  function saveGender(g) { localStorage.setItem(VOICE_GENDER_KEY, g === 'm' ? 'm' : 'f'); }
  // 资源版本号：与 tools/tts_adv.py 的 ASSET_V 保持一致；改语音后同步改这里
  const ASSET_V = '20260725a';
  // 风格化语音文件解析：升阶版只烤 humor2 一档，按性别拼后缀 _humor2(女) / _humor2_m(男)
  function vfile(base) { const g = loadGender() === 'm' ? '_m' : ''; return 'audio/' + base + '_humor2' + g + '.mp3?v=' + ASSET_V; }
  // 课程真人语音映射：按性别选女版(AUDIO_MAP)或男版(AUDIO_MAP_M)
  function audioMapOf(id) {
    if (loadGender() === 'm' && window.AUDIO_MAP_M && window.AUDIO_MAP_M[id]) return window.AUDIO_MAP_M[id];
    return (window.AUDIO_MAP && window.AUDIO_MAP[id]) || null;
  }
  // 鼓励语音(AUDIO_LEARN)：按性别选
  function audioLearn(key) {
    const M = loadGender() === 'm' ? window.AUDIO_LEARN_M : window.AUDIO_LEARN;
  ...
  // 分类：chapter 章勋章(18) / grad 毕业大章(1) / process 过程徽章(3) / exam 阶段考·毕业考过关章(5) — 共 27 枚
  const CHAPTER_BADGES = [
    { p: 1,  ch: 0,  cat: 'chapter', badge: '🌀', title: '异步启航章',   desc: '完成「异步爬虫基础」全部课程' },
    { p: 2,  ch: 1,  cat: 'chapter', badge: '🚀', title: '现代客户端章', desc: '完成「httpx 与现代客户端」全部课程' },
    { p: 3,  ch: 2,  cat: 'chapter', badge: '🧭', title: 'XPath 精准章',  desc: '完成「高效解析 XPath」全部课程' },
    { p: 4,  ch: 3,  cat: 'chapter', badge: '🎭', title: 'Playwright 章', desc: '完成「Playwright 进阶」全部课程' },
    { p: 5,  ch: 4,  cat: 'chapter', badge: '🔑', title: 'JS 逆向章',    desc: '完成「JS 逆向·参数签名」全部课程' },
    { p: 6,  ch: 5,  cat: 'chapter', badge: '🔤', title: '字体 OCR 章',  desc: '完成「字体·CSS 反爬与 OCR」全部课程' },
    { p: 7,  ch: 6,  cat: 'chapter', badge: '🧩', title: '验证码章',     desc: '完成「验证码与登录池」全部课程' },
    { p: 8,  ch: 7,  cat: 'chapter', badge: '🕷️', title: 'Scrapy 深入章', desc: '完成「Scrapy 框架深入」全部课程' },
    { p: 9,  ch: 8,  cat: 'chapter', badge: '🕸️', title: '分布式章',     desc: '完成「分布式·去重·管道」全部课程' },
    { p: 10, ch: 9,  cat: 'chapter', badge: '🗄️', title: '存储进阶章',   desc: '完成「存储进阶」全部课程' },
    { p: 11, ch: 10, cat: 'chapter', badge: '📱', title: 'App 抓包章',   desc: '完成「App 抓包与移动端」全部课程' },
    { p: 12, ch: 11, cat: 'chapter', badge: '⚙️', title: '部署监控章',   desc: '完成「调度·部署·监控」全部课程' },
    { p: 13, ch: 12, cat: 'chapter', badge: '⚖️', title: '合规深水章',   desc: '完成「合规深水区」全部课程' },
    { p: 14, ch: 13, cat: 'chapter', badge: '🎓', title: '毕业实战章',   desc: '完成「毕业综合 + 家长课」全部课程' },
    { p: 15, ch: 14, cat: 'chapter', badge: '🛡️', title: '代理 IP 章',   desc: '完成「代理 IP 池与反封禁」全部课程' },
    { p: 16, ch: 15, cat: 'chapter', badge: '📡', title: 'WebSocket 章', desc: '完成「WebSocket 实时爬取」全部课程' },
    { p: 17, ch: 16, cat: 'chapter', badge: '🎬', title: '音视频章',     desc: '完成「音视频与 m3u8 下载」全部课程' },
    { p: 18, ch: 17, cat: 'chapter', badge: '🚀', title: '前沿补充章',   desc: '完成「前沿补充」全部课程' }
  ];
  const GRAD_BADGE = { p: 19, cat: 'grad', badge: '👑', title: 'Python 爬虫大师', desc: '走完升阶全程：毕业项目A + 毕业项目B + 毕业考 全部完成' };
  const PROJECT_BADGE = { p: 28, cat: 'project', badge: '🏗️', title: '项目练习成就', desc: '完成初级 / 中级 / 高级 全部项目练习（写代码小项目）' };
  const PROCESS_BADGES = [
    { p: 20, cat: 'process', badge: '📅', title: '全勤勋章', desc: '全部课程（内容课 + 阶段考 + 毕业考）都走完' },
    { p: 21, cat: 'process', badge: '💯', title: '满分勋章', desc: '所有练习首次提交即全对' },
    { p: 22, cat: 'process', badge: '🛠️', title: '动手达人', desc: '所有课的动手任务全部勾掉' }
  ];
  const EXAM_BADGES = [
    { p: 23, id: 'exam1', cat: 'exam', badge: '🥇', title: '阶段考①过关', desc: '异步/httpx/XPath 三章综合测验首次正确率 ≥80%' },
    { p: 24, id: 'exam2', cat: 'exam', badge: '🥈', title: '阶段考②过关', desc: 'Playwright/JS逆向/字体OCR/验证码 四章综合测验 ≥80%' },
    { p: 25, id: 'exam3', cat: 'exam', badge: '🥉', title: '阶段考③过关', desc: 'Scrapy/分布式/存储/App 四章综合测验 ≥80%' },
    { p: 26, id: 'exam4', cat: 'exam', badge: '🏅', title: '阶段考④过关', desc: '部署监控/合规/代理/WebSocket/音视频 五章综合测验 ≥80%' },
    { p: 27, id: 'exam5', cat: 'exam', badge: '👑', title: '毕业考过关', desc: '升阶全程（ch1-18）+ 毕业项目 综合测验 ≥80%' }
  ];
  function allAwardDefs() {
    return CHAPTER_BADGES.concat([GRAD_BADGE]).concat([PROJECT_BADGE]).concat(PROCESS_BADGES).concat(EXAM_BADGES)
      .sort(function (a, b) { return a.p - b.p; });
  }
  // ---- 各类达成判定 ----
  function chapterDone(chIdx) {
    const ch = data.chapters[chIdx]; if (!ch) return false;
    // 家长课(forParent)孩子端无法"完成"，不纳入章节完成判定，避免阻断章勋章
    return ch.lessons.filter(function (l) { return !l.forParent; })
      .every(function (l) { return progress[l.id] && progress[l.id].done; });
  }
  ...
  function allCodePracticeDone() {
    let store = {};
    try { store = JSON.parse(localStorage.getItem('code_practice_done_v1') || '{}'); } catch (e) {}
    const kinds = ['projects', 'advanced', 'hard'];
    return kinds.every(function (k) {
      const tier = (store[k] || {});
      const total = (window.CODE_PRACTICE && window.CODE_PRACTICE[k] || []).length;
      let c = 0; for (const key in tier) if (tier[key]) c++;
  ...
  function checkAllBadges() {
    ...
    if (progress['gradA'] && progress['gradA'].done && progress['gradB'] && progress['gradB'].done && examRatio('exam5') >= 0.8) grant(GRAD_BADGE);
    if (allCodePracticeDone()) grant(PROJECT_BADGE);
  ...
  // 评价话术选音：升阶版只烤 humor2 一档，性别已 baked 进路径（AUDIO_MAP/AUDIO_MAP_M 二选一），此处不再追加后缀
  function learnSeg(s) {
    if (!s) return s;
    const pick = Array.isArray(s) ? s[Math.floor(Math.random() * s.length)] : s;
    if (pick && pick.src) return { src: pick.src };
  ...
  // 英文单词朗读兜底（预生成 mp3 缺失时才用）：按所选音色挑男/女声
  function speakEnTop(text) {
    ...
    u.lang = 'en-US';
    const vs = speechSynthesis.getVoices() || [];
    const wantMale = loadGender() === 'm';
    const v = vs.find((x) => (wantMale ? /male/i : /female/i).test(x.name) && /^en(-|_)/i.test(x.lang)) ||
              vs.find((x) => /^en(-|_)/i.test(x.lang)) || null;
    if (v) u.voice = v;
    u.rate = 0.92; u.pitch = wantMale ? 0.9 : 1.0;
    speechSynthesis.speak(u);
  }
  ...
  function playLearningReport(les, btn) {
    ...
    // 真人语音：用预生成片段拼接（向导点评部分按家长所选音色切换）
    const map = audioMapOf(les.id);
    if (map && map.takeaway) {
...
-----END PUBLIC KEY-----`;
  ...
  function getMachineId() {
    ...
    let mid = null;
    try { mid = localStorage.getItem('wb_mid_v1'); } catch (e) {}
  ...
  // 首页视图状态：'hub' 枢纽 / 'parent' 家长课程列表 / 'student' 学生课程列表 / 'student-zone' 学生园地（内置荣誉架）/ 'grad' 毕业典礼回顾
  ...
  function renderHub() {
    ...
    const zones = [
      ...
      { key: 'student-zone', icon: '🎮', color: '#5bc0a8', title: '学生园地', desc: '练习小站：单词本、练字、项目练习与荣誉架，随时巩固' },
  ...
  // 学生园地：练习小站（保留面板；已移除「项目练习成就」面板，成就改为荣誉墙徽章）
  function renderStudentZone() {
    ...
    // 荣誉架：勋章墙，已得的亮起、未得的显示剪影（含「项目练习成就」徽章）
    ...
    const tools = [
      ...
      { key: 'grad', icon: '🎓', color: '#ffcf3c', title: '毕业典礼回顾', desc: '毕业实战项目 A·高并发异步采集 / B·分布式+ES 入库，把整门课串成真项目', act: () => renderGradHub() },
  ...
  // 毕业典礼回顾：毕业实战项目枢纽（从学生园地「毕业典礼回顾」进入）
  function renderGradHub() {
    ...
    head.innerHTML = '<button class="back-home" id="back-grad">← 返回学生园地</button>' +
      '<h2 class="list-title">🎓 毕业典礼回顾 · 毕业实战项目</h2>';
    home.appendChild(head);
    // 毕业项目只从「学生园地」进入，返回固定回学生园地（避免打开项目后再返回陷入死循环）
    $('#back-grad').onclick = () => { homeViewName = 'student-zone'; renderHome(); };
    const grid = document.createElement('div');
    grid.className = 'zone-grid';
    const projs = [
      { id: 'gradA', icon: '🚀', color: '#3cc6ff', title: '项目 A · 高并发异步采集', desc: '把前 13 章串起来：用 asyncio+aiohttp+信号量+退避，做一个能并发抓千万页的异步采集器' },
      { id: 'gradB', icon: '🗄️', color: '#9b6bff', title: '项目 B · 分布式 + ES 入库', desc: '在 A 上加难度：多机分布式抓取，写入 Elasticsearch 支持检索' }
    ];
    projs.forEach(function (p) {
      ...
      tile.onclick = function () { if (g) openLesson(g); else toast('该毕业项目未找到，请重新下载最新版课程包'); };
      grid.appendChild(tile);
    });
    home.appendChild(grid);
    refreshScrollFab();
  ...
  function showParentPanelContent() {
    ...
    showModal(
      ...
      '</div>' +
      // 语音风格固定「幽默·梗王」；音色提供女声/男声可选
      '<div class="pp-section pp-theme-style">' +
        '<div class="pp-h"><span class="pp-ico">' + ICON.style + '</span>语音音色</div>' +
        '<div class="style-pick">' +
          '<button class="style-opt' + (loadGender() === 'f' ? ' on' : '') + '" data-gender="f">👩 女声（晓晓）</button>' +
          '<button class="style-opt' + (loadGender() === 'm' ? ' on' : '') + '" data-gender="m">👨 男声（云希）</button>' +
        '</div>' +
        '<p class="pp-tip-sm">讲解风格固定为「幽默·梗王」；这里切换真人语音的女声 / 男声。</p>' +
      '</div>' +
      // 课程解锁状态（可折叠）
    ...
    $('#modal').querySelectorAll('.style-opt').forEach((b) => {
      b.onclick = () => {
        ...
        toast('语音音色已切换为：' + (b.dataset.gender === 'm' ? '👨 男声（云希）' : '👩 女声（晓晓）'));
  ...
  function openLesson(les) {
    ...
    $('#back-btn').onclick = () => {
      ...
      $('#home-view').style.display = 'block';
      // 毕业项目(grad*)从学生园地进入，点返回直接回学生园地，避免绕回毕业枢纽
      if (typeof les.id === 'string' && les.id.indexOf('grad') === 0) homeViewName = 'student-zone';
      renderHome();
      updateProgress();
    };
  ...
  function renderLessonContent(les, c) {
    ...
    if (les.code) {
      if (les.codeCopyOnly) {
        // 浏览器跑不了的代码（asyncio / 分布式 / ES 等）：只展示 + 一键复制，提示去真机跑
        html += '<div class="code-copybox"><div class="cp-head"><span>🧪 完整代码（复制到你的电脑运行）</span>' +
          '<button class="btn ghost sm" id="copy-code-btn">📋 复制</button></div>' +
          '<textarea class="cp-input" id="cp-input" spellcheck="false" readonly>' + escapeHtml(les.code) + '</textarea>' +
          '<div class="cp-note">⚠️ 这段需要你电脑装好 Python 后再运行（这里不能直接跑 asyncio / 分布式 / ES）。把上面代码存成 <code>.py</code> 文件，终端执行 <code>python 文件名.py</code> 即可。</div></div>';
      } else {
    ...
    const copyBtn = c.querySelector('#copy-code-btn');
    if (copyBtn) copyBtn.onclick = function () {
      const ta = c.querySelector('#cp-input');
      if (!ta) return;
      ta.select();
      try { navigator.clipboard.writeText(ta.value); } catch (e) { try { document.execCommand('copy'); } catch (e2) {} }
      toast('已复制完整代码，去你电脑存成 .py 运行～');
    };
    ...
    if (stb) stb.onclick = function () {
      if (!currentLesson) return;
      const map = audioMapOf(currentLesson.id);
      if (map && map.takeaway) {
    ...
    if (!les.forParent && les.exercises && les.exercises.length) {
      ...
      if (sb) sb.onclick = () => {
        ...
        try {
          ...
          if (audioLearn('allCorrect')) encQ.push(learnSeg(audioLearn('allCorrect')));
  ...
  function renderExercises(exs, container) {
    ...
    exs.forEach((ex, idx) => {
      ...
      if (ex.type === 'choice') {
        ...
        const solCode = (window.CODING_SOLUTIONS && window.CODING_SOLUTIONS[currentLesson.id]) || ex.solution || ex.starter || '';
        inner += '<div class="ex-code-wrap">' +
          ...
          '<button class="btn ghost" data-act="show-sol" data-idx="' + idx + '">👀 参考答案</button>' +
          '</div>' +
          '<canvas class="ex-code-canvas" id="ex-cv-' + idx + '" width="260" height="200" style="display:none"></canvas>' +
          '<pre class="ex-code-output" id="ex-out-' + idx + '">点"运行代码"看结果～</pre>' +
          '<div class="ex-sol" id="ex-sol-' + idx + '" style="display:none">' +
            '<div class="ex-sol-head"><span>💡 参考答案</span>' +
            '<button class="btn ghost sm" data-act="copy-sol" data-idx="' + idx + '">📋 复制</button></div>' +
            '<pre class="ex-sol-code">' + escapeHtml(solCode) + '</pre>' +
          '</div>' +
        '</div>';
      } else if (ex.type === 'tap') {
    ...
    wrap.querySelectorAll('button[data-act]').forEach((btn) => {
      btn.onclick = () => {
        ...
        if (btn.dataset.act === 'check-choice') {
          ...
          fb.className = 'ex-feedback correct';
          fb.innerHTML = '✅ 已完成';
        } else if (btn.dataset.act === 'show-sol') {
          const sol = wrap.querySelector('#ex-sol-' + idx);
          if (sol) {
            sol.style.display = (sol.style.display === 'none') ? 'block' : 'none';
            if (sol.style.display === 'block') recordFirstAttempt(idx, 'viewed-sol');
          }
        } else if (btn.dataset.act === 'copy-sol') {
          const code = (window.CODING_SOLUTIONS && window.CODING_SOLUTIONS[currentLesson.id]) || ex.solution || ex.starter || '';
          const legacy = () => {
          ...
          if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(code).then(() => toast('已复制参考答案')).catch(() => { legacy(); toast('已复制参考答案'); });
          } else { legacy(); toast('已复制参考答案'); }
